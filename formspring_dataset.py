...
with open('Formspring.tsv', 'wt', encoding="utf-8",newline='') as out_file:
    tsv_writer = csv.writer(out_file, delimiter='\t', lineterminator='\n')
    tsv_writer.writerow(['userid', 'post', 'bully', 'severity'])
    for i in range(1,len(entries)):
        tsv_writer.writerow([users[i], posts[i], majority_binary[i], average_severity[i]])
