...
def get_data(fileName):  
    ...
    image_name = []
    for i in range(sheet.nrows):
        Real_list.append((sheet.cell_value(i, 0)))
...
def main():
    OCR_list, Real_list, image_name = get_data("Evaluate OCR - TWIT.xlsx")
    ...
    with open('Output.tsv', 'wt', encoding='utf-8') as out_file:
        ...
        for i in range(0, len(OCR_clean)):
            ...
            percentage = (1 - (mismatches / Real_clean_len)) * 100   #gets percentage of mismatches in user transcribed text
            tsv_writer.writerow([Real_clean[i], OCR_clean[i], mismatches, Real_clean_len, round(percentage, 3)])
